...
- **DeepSeek AI相关API（递减定价版）**：
  - GET /api/v1/ai/deepseek/price - 获取当前用户AI解读价格
  ...
  - GET /api/v1/ai/deepseek/purchase-history - 获取用户购买历史
  - POST /api/v1/ai/deepseek/config - 配置DeepSeek API参数（管理员）
- **支付相关API**：
  - POST /api/v1/payment/create-order - 创建支付订单（支持动态定价）
  ...
  - POST /api/v1/giftcode/activate - 激活礼品码
  - GET /api/v1/giftcode/status - 获取用户礼品码状态\n  - GET /api/v1/giftcode/remaining - 获取剩余免费次数
  - POST /api/v1/giftcode/validate - 验证礼品码有效性
- 历史记录相关：
...
- **后台管理API（简化管理员认证）**：
  - GET /api/v1/admin/check-permission - 检查管理员权限
  ...
  - PUT /api/v1/admin/system/giftcode-switch - 控制礼品码功能开关
  - GET /api/v1/admin/security/monitor - 获取安全监控数据
  - GET /api/v1/admin/logs/list - 获取系统日志
  - GET /api/v1/admin/audit/logs - 获取管理员操作审计日志
  - GET /api/v1/admin/ai/pricing-stats - 获取AI解读定价统计
  - **礼品码管理API**：
    - POST /api/v1/admin/giftcode/generate - 生成礼品码
    - GET /api/v1/admin/giftcode/list - 获取礼品码列表\n    - GET /api/v1/admin/giftcode/:code/details - 获取礼品码详情
    - PUT /api/v1/admin/giftcode/:code/disable - 禁用礼品码
    - PUT /api/v1/admin/giftcode/:code/enable - 启用礼品码
    - PUT /api/v1/admin/giftcode/:code/reset - 重置礼品码使用次数
    - DELETE /api/v1/admin/giftcode/:code - 删除礼品码
    - GET /api/v1/admin/giftcode/stats - 获取礼品码统计数据
\n### 5.2 本地存储数据结构
```javascript
// IndexedDB 测试记录结构（递减定价+简化管理员+礼品码版）
{
  id: 'test_timestamp',
  userId: 'user_email',
  isAdmin: false, // 管理员标识
  giftcodeInfo: {
    hasGiftcode: true, // 是否使用礼品码
    giftcodeUsed: 'ABC123XYZ', // 使用的礼品码
    remainingFreeUses: 12, // 剩余免费次数
    totalFreeUses: 15 // 总免费次数\n  },
  ...
  aiAnalysis: {
    ...
    paidPrice: 0.00, // 实际支付价格（礼品码用户为0）
    paymentMethod: 'giftcode', // 支付方式：giftcode/alipay/wechat
    orderId: 'giftcode_20251112_001',
...
// MongoDB 礼品码集合结构
{
  ...
  totalUses: 15, // 总可用次数
  remainingUses: 15, // 剩余可用次数
  activatedUsers: [ // 激活用户列表
    {
      userId: 'user@example.com',
      activatedAt: '2025-11-12T10:00:00Z',
      usedCount: 3,
      lastUsedAt: '2025-11-12T15:30:00Z'
    }
  ],
  createdBy: 'admin@example.com', // 创建管理员
  createdAt: '2025-11-12T09:00:00Z',
  expiresAt: null, // 过期时间（可选）
  settings: {
    multiUser: true, // 是否允许多用户使用
    maxUsers: 100, // 最大用户数限制
    description: '新用户体验礼品码' // 礼品码描述
  },
  stats: {
    totalActivations: 1, // 总激活次数\n    totalUsages: 3, // 总使用次数
    conversionRate: 0.0// 转化率（免费转付费）
  }
}\n```

### 5.5 数据格式\n统一使用JSON格式进行请求和响应，包含标准化的错误处理机制、支付状态管理、递减定价逻辑、礼品码验证和安全验证。
...
- **礼品码信息保护**：礼品码状态和使用记录加密存储
\n### 9.2 数据同步策略
- 本地优先，云端备份的混合模式
- 离线模式下完整功能支持
- 网络恢复时自动同步机制
- **支付数据安全**：支付信息仅在必要时传输，不做本地持久化存储
- **礼品码数据同步**：礼品码使用状态实时同步，确保数据一致性
...
- **递减定价展示**：清晰显示当前价格、原价对比和优惠幅度
- **购买激励**：展示'越用越便宜'的定价策略，鼓励多次使用
- **购买流程优化**：一键购买，支付成功后自动生成AI解读\n- **内容呈现优化**：AI解读内容采用渐进式展示，提升阅读体验
- **服务反馈机制**：用户可对AI解读质量进行评价和反馈
- **历史服务管理**：用户可查看所有购买的AI解读历史、支付价格和重新下载
- **礼品码用户体验**：礼品码用户享受完全免费的AI解读服务，界面突出免费标识
...
### 10.6 管理员体验优化
- **无缝管理员体验**：指定邮箱用户注册后自动获得管理员权限，无需额外设置
- **直观的权限提示**：管理员用户界面清晰标识管理员身份
- **快速访问**：管理员可一键切换到后台管理界面
- **礼品码管理体验**：
  - 直观的礼品码生成向导，支持批量生成
  - 礼品码列表的高效管理和搜索功能
  - 实时的礼品码使用统计和分析报告
...
- **礼品码策略**：
  ...
  - 用于新用户获取、市场推广、合作伙伴奖励
  - 礼品码用户转化为付费用户的长期价值挖掘
- **策略优势**：鼓励用户多次使用，提高用户粘性和长期价值
- **促销策略**：新用户首次体验优惠，节假日特价活动，礼品码限时发放
...
- **定价策略优化**：根据用户行为数据调整递减定价参数
- **安全运营**：建立7x24小时安全监控和响应机制
- **管理员运营**：简化管理员设置流程，提高运营效率
- **礼品码运营策略**：
  - 根据市场活动和用户获取需求灵活发放礼品码
  - 监控礼品码使用情况，及时调整发放策略
  - 建立礼品码用户的精准营销和转化体系
  - 通过礼品码数据分析优化产品功能和用户体验