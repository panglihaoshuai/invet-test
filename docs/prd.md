...
### 1.2 系统描述
这是一个Web应用程序，用于评估用户的人格特质、数学/金融能力以及风险偏好，并推荐合适的投资策略。系统通过科学的测评方法和欧几里得距离算法，为用户提供个性化的投资建议和详细报告。支持本地化数据存储和历史记录回顾功能。系统提供标准版和完整版两种测试模式，完整版融合游戏化测试体验，测试时长20-30分钟。
...
- **完整版测试**：融合游戏化体验的深度测试，测试时长20-30分钟
...
- 结合交易特征数据和游戏化测试数据进行多维度投资性格画像构建
- 支持趋势跟踪、波段交易、价值投资等多种投资风格匹配
- **游戏数据权重算法**：将游戏化测试结果与传统问卷结果进行加权融合
...
- **游戏回放功能**：保存游戏过程数据，支持历史游戏表现回放

### 2.7 报告生成与下载
- 自动生成包含评估结果和投资建议的PDF报告
- **完整版报告增强**：包含游戏化测试分析、行为模式图表、风险偏好热力图
- 支持报告预览和下载功能
- 历史报告重新生成和下载\n- 本地报告缓存机制
...
├── IndexedDB: 测试历史记录、游戏回放数据、报告缓存
└── SessionStorage: 当前测试进度、游戏状态
\n[后端云存储]
├── MongoDB: 用户账户信息、统计数据、游戏行为分析
└── 临时存储: PDF报告生成缓存
```
...
```
[用户浏览器]<--> [前端SPA (React)] <--> [游戏引擎模块] <--> [本地存储层(IndexedDB/LocalStorage)]
                     |\n                     v
[REST API网关 (Express)] <--> [后端模块 (Node.js)] <--> [游戏数据分析引擎]
                     |
                     v
...
- 测试相关：
  ...
  - **游戏相关新增**：
    ...
    - GET /api/v1/games/replay/:gameId - 获取游戏回放数据
- 历史记录相关：
      ...
      rounds: [
        {
          ...
          exploded: false,
          coinsEarned: 120,
          decisionTime: [800, 750, 900, ...] // 每次决策用时
        }
      ],
      totalCoins: 450,
      riskScore: 0.75
...
- **游戏UI专用色**：金黄色 (#FFD700) 用于金币显示，橙红色 (#FF4500) 用于爆炸警告
